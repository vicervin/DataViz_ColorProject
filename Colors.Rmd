...
xspace<-cbind(1:100,1:100)
plot(xspace,type="n",axes=FALSE,xlab = " Mean of measured(right) vs Master(left) colors ",ylab = "")
...
for( i in 1:8){
  for( j in 1:8){
    ...
    tryCatch({
      cola <- rgb(b[1,][1], b[1,][2], b[1,][3])
      rect(xleft+width/2+spacing/8,ybottom,xleft+width,ybottom+width,col=cola,border = FALSE)
    }, warning = function(w) {
    }, error = function(e) {print(c(i,j,b))})
  }
}
...
title(" Furthest outlying colorcell comparison", sub = "Top: Mean of Measured, Bottom: Master")
```
...
title("Colorcell with the least difference", sub = "Top: Mean of Measured, Bottom: Master")
```
...
heatmap(as.matrix(x), scale="column", col = terrain.colors(256), margins=c(8,5), main = "Heatmap of Lab values (master & measured)")
```
